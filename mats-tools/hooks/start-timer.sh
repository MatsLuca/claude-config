...
# Start still im Detail (Terminal-Zeile und Modell-Kontext nur mit MATS_START_TIMER_SHOW=1 — der
# Normalfall ist das Log; LatexTerm zeigt die gefühlte Zeit als Live-Zähler auf dem Start-Vorhang) nach
...
# Normalfall: nur das Log — kein Terminal, kein Modell-Kontext (bis 22.09. ging die Zeile bei jedem
# Start als additionalContext an Claude, ~100 Token, die keine Session brauchte). Sichtbar nur mit
# MATS_START_TIMER_SHOW=1 oder --self.
[ "${MATS_START_TIMER_SHOW:-0}" = 1 ] || [ "$SELF" = 1 ] || exit 0
